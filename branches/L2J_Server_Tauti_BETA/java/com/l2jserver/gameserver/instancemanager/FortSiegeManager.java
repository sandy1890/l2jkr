...
public class FortSiegeManager {
	...
	public final boolean checkIfOkToSummon(L2Character activeChar, boolean isCheckOnly) {
		...
		if ((fort == null) || (fort.getFortId() <= 0)) {
			/*
			 * Move To MessageTable For L2JTW text = "You must be on fort ground to summon this";
			 */
			text = 174;
		} else if (!fort.getSiege().getIsInProgress()) {
			/*
			 * Move To MessageTable For L2JTW text = "You can only summon this during a siege.";
			 */
			text = 175;
		} else if ((player.getClanId() != 0) && (fort.getSiege().getAttackerClan(player.getClanId()) == null)) {
			/*
			 * Move To MessageTable For L2JTW text = "You can only summon this as a registered attacker.";
			 */
			text = 176;
		} else {
			return true;
	...
	public boolean checkIfCanPickup(L2PcInstance player) {
		...
		if (player.isCombatFlagEquipped()) {
			/*
			 * Move To MessageTable For L2JTW player.sendPacket(sm);
			 */
