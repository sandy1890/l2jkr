...
public class MultiSell {
	 ...
	 * in order to add a certain amount of adena to the ingredients 4) additional product and ingredient multipliers
	 * @param listId
	 * @param player
	 * @param npc
	 * @param inventoryOnly
		...
		switch (id) {
			case PC_BANG_POINTS:
				if (player.getPcBangPoints() < amount) {
					player.sendPacket(SystemMessageId.SHORT_OF_ACCUMULATED_POINTS);
					break;
				}
				return true;
