...
public final class L2AuctioneerInstance extends L2Npc {
	...
	public void onBypassFeedback(L2PcInstance player, String command) {
		...
		if (condition <= COND_ALL_FALSE) {
			// TODO: html
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("Wrong conditions.");
			 */
			...
			if (actualCommand.equalsIgnoreCase("auction")) {
				...
				try {
					...
					try {
						...
						/*
						 * Move To MessageTable For L2JTW player.sendMessage("Invalid bid!");
						 */
						player.sendMessage(535);
					}
				} catch (Exception e) {
					/*
					 * Move To MessageTable For L2JTW player.sendMessage("Invalid auction duration!");
					 */
				...
				try {
					...
					/*
					 * Move To MessageTable For L2JTW player.sendMessage("Invalid auction");
					 */
				...
				try {
					...
					if (a != null) {
						...
						html.replace("%AGIT_AUCTION_END%", String.valueOf(format.format(a.getEndDate())));
						/*
						 * Move To MessageTable For L2JTW html.replace("%AGIT_AUCTION_REMAIN%", String.valueOf((a.getEndDate()- System.currentTimeMillis())/3600000)+" hours "+String.valueOf((((a.getEndDate() - System.currentTimeMillis()) / 60000) % 60))+" minutes");
						 */
				...
				for (int j = 1; j <= npage; j++) {
					...
					items.append(j);
					/*
					 * Move To MessageTable For L2JTW items.append("\"> Page ");
					 */
				...
				if (AuctionManager.getInstance().getAuction(player.getClan().getHideoutId()) != null) {
					...
					/*
					 * Move To MessageTable For L2JTW player.sendMessage("Your auction has been canceled");
					 */
