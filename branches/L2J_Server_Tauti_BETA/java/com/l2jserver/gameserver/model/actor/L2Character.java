...
public abstract class L2Character extends L2Object {
	...
	public static final byte ZONE_JUMP = 23; // rocknow-God-Awaking
	
	private final byte[] _zones = new byte[24]; // rocknow-God-Awaking
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Set the _template of the L2Character</li> <li>Set _overloaded to false (the character can take more items)</li><BR>
	 * <BR>
	 ...
	 * <BR>
	 * @param player The L2Character that attacks this one <B><U> Overridden in </U> :</B><BR>
	 * <BR>
	 *            <li>L2Attackable : Add to list only for attackables, not all other NPCs</li><BR>
	 * <BR>
	 */
	...
	public void teleToLocation(Location loc, int randomOffset) {
		...
		if (isPlayer() && DimensionalRiftManager.getInstance().checkIfInRiftZone(getX(), getY(), getZ(), true)) { // true -> ignore waiting room :)
			...
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("You have been sent to the waiting room.");
			 */
	...
	protected void doAttack(L2Character target) {
		...
		if (getActingPlayer() != null) {
			if (getActingPlayer().inObserverMode()) {
				...
				(getActingPlayer().getSiegeState() > 0) &&
				isInsideZone(L2Character.ZONE_SIEGE) &&
				(target.getActingPlayer().getSiegeState() == getActingPlayer().getSiegeState()) &&
				...
				sendPacket(ActionFailed.STATIC_PACKET);
				return;
			} else if (target.isInsidePeaceZone(getActingPlayer())) { // Checking if target has moved to peace zone
				getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE);
				sendPacket(ActionFailed.STATIC_PACKET);
				return;
			}
		...
		if (weaponItem != null) {
			...
			ssGrade = weaponItem.getItemGradeRPlus();
			// l2jtw end
		}
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Calculate if hit is missed or not</li> <li>Consume arrows</li> <li>If hit isn't missed, calculate if shield defense is efficient</li> <li>If hit isn't missed, calculate if hit is critical</li> <li>If hit isn't missed, calculate physical damages</li> <li>If the L2Character is a
	...
	private boolean doAttackHitByBow(Attack attack, L2Character target, int sAtk, int reuse) {
		...
		if (isPlayer()) {
			...
			SetupGauge sg = new SetupGauge(SetupGauge.RED_MINI, sAtk + reuse); // rocknow-God-Awaking
			sendPacket(sg);
		}
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Calculate if hit is missed or not</li> <li>Consume bolts</li> <li>If hit isn't missed, calculate if shield defense is efficient</li> <li>If hit isn't missed, calculate if hit is critical</li> <li>If hit isn't missed, calculate physical damages</li> <li>If the L2Character is a
	 * L2PcInstance, Send a Server->Client packet SetupGauge</li> <li>Create a new hit task with Medium priority</li> <li>Calculate and set the disable delay of the crossbow in function of the Attack Speed</li> <li>Add this hit to the Server-Client packet Attack</li><BR>
	 * <BR>
	 * @param attack Server->Client packet Attack in which the hit will be added
	 * @param target The L2Character targeted
	 * @param sAtk The Attack Speed of the attacker
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get all visible objects in a spherical area near the L2Character to obtain possible targets</li> <li>If possible target is the L2Character targeted, launch a simple attack against it</li> <li>If possible target isn't the L2Character targeted but is attackable, launch a simple attack
	 * against it</li><BR>
	 * <BR>
	 * @param attack Server->Client packet Attack in which the hit will be added
	 * @param target
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Verify the possibility of the the cast : skill is a spell, caster isn't muted...</li> <li>Get the list of all targets (ex : area effects) and define the L2Charcater targeted (its stats will be used in calculation)</li> <li>Calculate the casting time (base + modifier of MAtkSpd), interrupt
	 * time and re-use delay</li> <li>Send a Server->Client packet MagicSkillUser (to display casting animation), a packet SetupGauge (to display casting bar) and a system message</li> <li>Disable all skills during the casting time (create a task EnableAllSkills)</li> <li>Disable the skill during
	 * the re-use delay (create a task EnableSkill)</li> <li>Create a task MagicUseTask (that will call method onMagicUseTimer) to launch the Magic Skill at the end of the casting time</li><BR>
	 * <BR>
		...
		if (effectWhileCasting) {
			...
			if (skill.getItemConsumeId() > 0) {
				if (!destroyItemByItemId("Consume", skill.getItemConsumeId(), skill.getItemConsume(), null, true)) {
					...
					sendPacket(SystemMessageId.THERE_ARE_NOT_ENOUGH_NECESSARY_ITEMS_TO_USE_THE_SKILL); // Update by rocknow
					if (simultaneously) {
						setIsCastingSimultaneouslyNow(false);
					} else {
						setIsCastingNow(false);
	...
	/*
	 * (non-Javadoc)
	 * @see com.l2jserver.gameserver.model.L2Object#getKnownList()
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.l2jserver.gameserver.model.L2Object#initKnownList()
	 */
	@Override
	...
	/**
	 * Initializes the CharStat class of the L2Object, is overwritten in classes that require a different CharStat Type. Removes the need for instanceof checks.
	 */
	...
	/**
	 * Initializes the CharStatus class of the L2Object, is overwritten in classes that require a different CharStatus Type. Removes the need for instanceof checks.
	 */
	...
	/*
	 * (non-Javadoc)
	 * @see com.l2jserver.gameserver.model.L2Object#getPosition()
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see com.l2jserver.gameserver.model.L2Object#initPosition()
	 */
	@Override
	 ...
	 * set of the L2Character (remove previous stacked effect Funcs if necessary)</li> <li>If this effect has NOT higher priority in its Stack Group, set the effect to Not In Use</li> <li>Update active skills in progress icons on player client</li><BR>
	 * @param newEffect
	 */
	 ...
	 * progress icons on player client</li><BR>
	 * @param effect
	 */
	...
	public final void startAbnormalEffect(AbnormalEffect mask) {
		...
		// l2jtw add start
		// _AbnormalEffectsList.add(mask.getId());
		// l2jtw add end
	...
	public final void startSpecialEffect(AbnormalEffect[] mask) {
		for (AbnormalEffect special : mask) {
			...
			// l2jtw add start
			// _SpecialEffectsList.add(special.getId());
			// l2jtw add end
		}
		updateAbnormalEffect();
	}
	...
	public final void startSpecialEffect(AbnormalEffect mask) {
		...
		// l2jtw add start
		// _SpecialEffectsList.add(mask.getId());
		// l2jtw add end
		updateAbnormalEffect();
	}
	...
	public final void startAbnormalEffect(int mask) {
		...
		// l2jtw add start
		// _AbnormalEffectsList.add(AbnormalEffect.getAbnormalId(mask));
		// l2jtw add end
		updateAbnormalEffect();
	}
	...
	public final void startSpecialEffect(int mask) {
		...
		// l2jtw add start
		// _SpecialEffectsList.add(AbnormalEffect.getSpecialId(mask));
		// l2jtw add end
		updateAbnormalEffect();
	}
	...
	public final void stopAbnormalEffect(AbnormalEffect mask) {
		_AbnormalEffects &= ~mask.getMask();
		// l2jtw add start
		// _AbnormalEffectsList.remove(mask.getId());
		// l2jtw add end
		updateAbnormalEffect();
	}
	...
	public final void stopSpecialEffect(AbnormalEffect[] mask) {
		for (AbnormalEffect special : mask) {
			_SpecialEffects &= ~special.getMask();
			// l2jtw add start
			// _SpecialEffectsList.remove(special.getId());
			// l2jtw add end
		}
		updateAbnormalEffect();
	}
	...
	public final void stopSpecialEffect(AbnormalEffect mask) {
		_SpecialEffects &= ~mask.getMask();
		// l2jtw add start
		// _SpecialEffectsList.remove(mask.getId());
		// l2jtw add end
		updateAbnormalEffect();
	}
	...
	public final void stopAbnormalEffect(int mask) {
		_AbnormalEffects &= ~mask;
		// l2jtw add start
		// _AbnormalEffectsList.remove(AbnormalEffect.getAbnormalId(mask));
		// l2jtw add end
		updateAbnormalEffect();
	}
	...
	public final void stopSpecialEffect(int mask) {
		_SpecialEffects &= ~mask;
		// l2jtw add start
		// _SpecialEffectsList.remove(AbnormalEffect.getSpecialId(mask));
		// l2jtw add end
		updateAbnormalEffect();
	}
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Delete a specified/all (if effect=null) Fake Death abnormal L2Effect from L2Character and update client magic icon</li> <li>Set the abnormal effect flag _fake_death to False</li> <li>Notify the L2Character AI</li><BR>
	 * <BR>
	 * @param removeEffects
	 */
	...
	public List<Integer> getEffectIdList() {
		...
		int ae = getAbnormalEffect();
		int se = getSpecialEffect();
		for (AbnormalEffect e : AbnormalEffect.values()) {
			if (e.getId() == 0) {
				continue;
			}
			if ((e != AbnormalEffect.STEALTH) || isGM()) {
	...
	protected void moveToLocation(int x, int y, int z, int offset) {
		...
		if ((Config.GEODATA > 0) && !isFlying() // flying chars not checked - even canSeeTarget doesn't work yet
			...
			if (((Config.GEODATA == 2) &&
				!(isL2Attackable() &&
				((L2Attackable) this).isReturningToSpawnPoint())) || (isPlayer() &&
				!(isInVehicle && (distance > 1500))) || (isSummon() &&
				!(getAI().getIntention() == AI_INTENTION_FOLLOW)) ||
	...
	*/
					...
					if (!Config.RAID_DISABLE_CURSE &&
						...
						if (skill.isMagic()) {
							...
							if (tempSkill != null) {
								...
								_log.log(Level.WARNING, "스킬:4215, 레벨:1 - 데이터 팩에 존재하지 않습니다.");
							}
						} else {
							...
							if (tempSkill != null) {
								...
								_log.log(Level.WARNING, "스킬:4515, 레벨:1 - 데이터 팩에 존재하지 않습니다.");
							}
						}
						return;
					}
	 ...
	 * <B><U> Overridden in </U> :</B><BR>
	 * <BR>
	 * <li>L2PcInstance <li>L2ServitorInstance <li>L2PetInstance</li><BR>
	 * <BR>
	 * @param target
	 * @param damage
	 * @param mcrit
