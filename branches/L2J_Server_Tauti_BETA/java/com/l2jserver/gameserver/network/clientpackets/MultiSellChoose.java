...
public class MultiSellChoose extends L2GameClientPacket {
	...
	/*
	 * @SuppressWarnings("unused") private int _unk1;
	 * @SuppressWarnings("unused") private int _unk2;
	 * @SuppressWarnings("unused") private int _unk3;
	 * @SuppressWarnings("unused") private int _unk7;
	 * @SuppressWarnings("unused") private int _unk4;
	 * @SuppressWarnings("unused") private int _unk5;
	 * @SuppressWarnings("unused") private int _unk6;
	 * @SuppressWarnings("unused") private int _unk8;
	 * @SuppressWarnings("unused") private int _unk9;
	 * @SuppressWarnings("unused") private int _unk10;
	 * @SuppressWarnings("unused") private int _unk11;
	 */
	...
	protected void readImpl() {
		...
		_amount = readQ();
		/*
		 * _unk1 = readH(); _unk2 = readD(); _unk3 = readD(); _unk4 = readH(); // elemental attributes _unk5 = readH(); // elemental attributes _unk6 = readH(); // elemental attributes _unk7 = readH(); // elemental attributes _unk8 = readH(); // elemental attributes _unk9 = readH(); // elemental
		 * attributes _unk10 = readH(); // elemental attributes _unk11 = readH(); // elemental attributes
		 */
	}
				...
				try {
					for (Ingredient e : entry.getIngredients()) {
						if (e.getItemId() < 0) {
							...
							/*
							 * if (itemToTake.isEquipped()) { //this is a cheat, transaction will be aborted and if any items already taken will not be returned back to inventory! _log.severe("Character: " + player.getName() +
							 * " is trying to cheat in multisell, exchanging equipped item, merchatnt id:" + merchant.getNpcId()); player.setMultiSell(null); return; }
							 */
							...
							if (Config.ALT_BLACKSMITH_USE_RECIPES || !e.getMaintainIngredient()) {
								...
								if (itemToTake.isStackable()) {
									...
									{
										/*
										 * NOTE: There are 2 ways to achieve the above goal. 1) Get all items that have the correct itemId, loop through them until the lowest enchantment level is found. Repeat all this for the next item until proper count of items is reached. 2) Get all items that
										 * have the correct itemId, sort them once based on enchantment level, and get the range of items that is necessary. Method 1 is faster for a small number of items to be exchanged. Method 2 is faster for large amounts. EXPLANATION: Worst case scenario for
										 * algorithm 1 will make it run in a number of cycles given by: m*(2n-m+1)/2 where m is the number of items to be exchanged and n is the total number of inventory items that have a matching id. With algorithm 2 (sort), sorting takes n*log(n) time and the
										 * choice is done in a single cycle for case b (just grab the m first items) or in linear time for case a (find the beginning of items with correct enchantment, index x, and take all items from x to x+m). Basically, whenever m > log(n) we have: m*(2n-m+1)/2 =
										 * (2nm-m*m+m)/2 > (2nlogn-logn*logn+logn)/2 = nlog(n) - log(n*n) + log(n) = nlog(n) + log(n/n*n) = nlog(n) + log(1/n) = nlog(n) - log(n) = (n-1)log(n) So for m < log(n) then m*(2n-m+1)/2 > (n-1)log(n) and m*(2n-m+1)/2 > nlog(n) IDEALLY: In order to best
										 * optimize the performance, choose which algorithm to run, based on whether 2^m > n if ( (2<<(e.getItemCount() * _amount)) < inventoryContents.length ) // do Algorithm 1, no sorting else // do Algorithm 2, sorting CURRENT IMPLEMENTATION: In general, it is
										 * going to be very rare for a person to do a massive exchange of non-stackable items For this reason, we assume that algorithm 1 will always suffice and we keep things simple. If, in the future, it becomes necessary that we optimize, the above discussion
