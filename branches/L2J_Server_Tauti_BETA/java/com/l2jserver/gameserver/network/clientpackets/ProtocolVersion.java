...
public final class ProtocolVersion extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		/*
		 * else if (_version < (_support - 50)) { LogRecord record = new LogRecord(Level.WARNING, "Protocol too old"); record.setParameters(new Object[]{_version, getClient()}); _logAccounting.log(record); _log.info(getClient() + MessageTable.Messages[1].getExtra(1) +
		 * MessageTable.Messages[1].getExtra(3) + MessageTable.Messages[1].getExtra(2)); KeyPacket pk = new KeyPacket(getClient().enableCrypt(),0); getClient().setProtocolOk(false); getClient().close(pk); } else if (_version > (_support + 50)) { LogRecord record = new LogRecord(Level.WARNING,
		 * "Protocol too new"); record.setParameters(new Object[]{_version, getClient()}); _logAccounting.log(record); _log.info(getClient() + MessageTable.Messages[1].getExtra(1) + MessageTable.Messages[1].getExtra(4) + MessageTable.Messages[1].getExtra(2)); KeyPacket pk = new
