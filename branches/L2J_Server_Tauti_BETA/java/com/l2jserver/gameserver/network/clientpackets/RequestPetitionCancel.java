		...
		if (PetitionManager.getInstance().isPlayerInConsultation(activeChar)) {
			...
			if (PetitionManager.getInstance().isPlayerPetitionPending(activeChar)) {
				if (PetitionManager.getInstance().cancelActivePetition(activeChar)) {
					...
					/*
					 * Move To MessageTable For L2JTW String msgContent = activeChar.getName() + " has canceled a pending petition."; AdminTable.getInstance().broadcastToGMs(new CreatureSay(activeChar.getObjectId(), Say2.HERO_VOICE, "Petition System", msgContent));
					 */
