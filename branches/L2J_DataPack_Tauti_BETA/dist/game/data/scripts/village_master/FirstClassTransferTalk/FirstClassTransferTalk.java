 ...
 * @author jurchiks
 * Update by pmq 19-09-2011 High Five Fix No Check Higher Classes
 */
		...
		switch (npc.getNpcId())
		{
			...
			case ALDENIA:
			{
				if (player.getRace() != Race.Human || player.getClassId().isMage())
					htmltext += "no.html";
				else if (player.getClassId().level() >= 0) // no check higher classes
			...
			case TOOK:
			{
				if (player.getRace() != Race.Orc)
					...
					if (player.getClassId().isMage() || player.getClassId().getId() == 49) // fix check Orc Mage
						htmltext += "mystic.html";
					else
						htmltext += "fighter.html";
				break;
			...
			case MOKA:
			{
				...
				if (player.getClassId().level() >= 0) // no check higher classes
					htmltext += "fighter.html";
				break;
			}
		}
