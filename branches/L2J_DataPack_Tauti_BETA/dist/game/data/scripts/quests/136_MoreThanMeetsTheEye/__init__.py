...
class Quest (JQuest) :
 ...
 def onTalk (self,npc,player):
   ...
   if id == State.COMPLETED :
      ...
      elif cond > 1 and cond < 5 :                  #增加:接下階段性任務後的對話
         htmltext = "30832-05_end.htm"              #增加:接下階段性任務後的對話
      elif cond == 5 :
         htmltext = "30832-06.htm"
      elif cond > 5 and cond < 9 :                  #增加:接下階段性任務後的對話
         htmltext = "30832-10_end.htm"              #增加:接下階段性任務後的對話
      elif cond == 9 :
         htmltext = "30832-11.htm"
   elif npcId == ERRICKIN :
      if cond == 2 :
         htmltext = "30701-01.htm"
      if cond == 3 :                                #增加:接下階段性任務後的對話
         htmltext = "30701-02_end.htm"              #增加:接下階段性任務後的對話
      if cond == 4 :
         ...
         st.set("cond","5")
         st.playSound("ItemSound.quest_middle")
      elif cond == 5 :                              #增加:接下階段性任務後的對話
         htmltext = "30701-03_end.htm"              #增加:接下階段性任務後的對話
   elif npcId == CLAYTON :
      if cond == 6 :
         htmltext = "30464-01.htm"
      if cond == 7 :                                #增加:接下階段性任務後的對話
         htmltext = "30464-02_end.htm"              #增加:接下階段性任務後的對話
      if cond == 8 :
         ...
         st.set("cond","9")
         st.playSound("ItemSound.quest_middle")
      elif cond == 9 :                              #增加:接下階段性任務後的對話
         htmltext = "30464-03_end.htm"              #增加:接下階段性任務後的對話
   return htmltext
