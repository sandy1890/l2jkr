...
class Quest (JQuest) :
	...
	def onAdvEvent (self,event,npc,player) :
		...
		if event == "32655-yes.htm" :
			...
			if int(event) == 9482 and f >= 326 :						# Recipe: Sealed Dynasty Breast Plate (60%)
				htmltext = self.onExchangeRequest(event,st,326)
			elif int(event) == 9483 and f >= 204 :						# Recipe: Sealed Dynasty Gaiter (60%)
				htmltext = self.onExchangeRequest(event,st,204)
			elif int(event) == 9484 and f >= 122 :						# Recipe: Sealed Dynasty Helmet (60%)
				htmltext = self.onExchangeRequest(event,st,122)
			elif int(event) == 9485 and f >= 82 :						# Recipe: Sealed Dynasty Gauntlet (60%)
				htmltext = self.onExchangeRequest(event,st,82)
			elif int(event) == 9486 and f >= 82 :						# Recipe: Sealed Dynasty Boots (60%)
				htmltext = self.onExchangeRequest(event,st,82)
			elif int(event) == 9487 and f >= 244 :						# Recipe: Sealed Dynasty Leather Armor (60%)
				htmltext = self.onExchangeRequest(event,st,244)
			elif int(event) == 9488 and f >= 153 :						# Recipe: Sealed Dynasty Leather Leggings (60%)
				htmltext = self.onExchangeRequest(event,st,153)
			elif int(event) == 9489 and f >= 122 :						# Recipe: Sealed Dynasty Leather Helmet (60%)
				htmltext = self.onExchangeRequest(event,st,122)
			elif int(event) == 9490 and f >= 82 :						# Recipe: Sealed Dynasty Leather Gloves (60%)
				htmltext = self.onExchangeRequest(event,st,82)
			elif int(event) == 9491 and f >= 82 :						# Recipe: Sealed Dynasty Leather Boots (60%)
				htmltext = self.onExchangeRequest(event,st,82)
			elif int(event) == 9497 and f >= 86 :						# Recipe: Sealed Dynasty Shield (60%)
				htmltext = self.onExchangeRequest(event,st,86)
