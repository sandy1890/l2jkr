...
public class NornilsGarden extends Quest {
	...
	private static final int INSTANCE_LVL_MIN = 17;
	...
	/*
	 * public String onAggroRangeEnter(L2Npc npc, L2PcInstance player, boolean isPet) { InstanceWorld tmpworld = InstanceManager.getInstance().getWorld(player.getInstanceId()); if (tmpworld instanceof NornilsWorld) { if (npc.getNpcId() == 18437) { ((L2Attackable) npc).clearAggroList();
	 * npc.getAI().setIntention(CtrlIntention.AI_INTENTION_IDLE); npc.setTarget(player); giveBuffs(player); //npc.doCast(SkillTable.getInstance().getInfo(4322,1)); //npc.doCast(SkillTable.getInstance().getInfo(4327,1)); //npc.doCast(SkillTable.getInstance().getInfo(4329,1));
	 * //npc.doCast(SkillTable.getInstance().getInfo(4324,1)); } } return null; }
	 */
		...
		if ((npc.getNpcId() == _garden_guard) && event.equalsIgnoreCase("enter_instance")) {
			...
			if (event.equalsIgnoreCase("32260-02.html") || event.equalsIgnoreCase("32261-02.html") || event.equalsIgnoreCase("32262-02.html")) {
				...
				if ((npc.getNpcId() == 32260) && (correct == 3)) {
					...
					// Door Open Send Message
					player.sendPacket(new ExShowScreenMessage("渾沌的未來...", 15000));
					htmltext = "渾沌的未來...";
				} else {
