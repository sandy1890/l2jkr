...
public final class RequestAcquireSkillInfo extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		/*
		 * l2jtw start final L2Npc trainer = activeChar.getLastFolkNPC(); if (!(trainer instanceof L2NpcInstance)) { return; } if (!trainer.canInteract(activeChar) && !activeChar.isGM()) { return; } l2jtw end
		 */
		...
		switch (skillType) {
			case Class: {
				/*
				 * l2jtw start if (trainer.getTemplate().canTeach(activeChar.getLearningClass())) { final int customSp = s.getCalculatedLevelUpSp(activeChar.getClassId(), activeChar.getLearningClass()); sendPacket(new AcquireSkillInfo(skillType, s, customSp)); }
				 */
			...
			case SubPledge: {
				...
				/*
				 * l2jtw start if (!(trainer instanceof L2SquadTrainer)) { return; }
				 */
				sendPacket(new AcquireSkillInfo(skillType, s));
				break;
			}
