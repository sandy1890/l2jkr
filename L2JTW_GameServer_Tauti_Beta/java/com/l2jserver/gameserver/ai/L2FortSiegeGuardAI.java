...
public class L2FortSiegeGuardAI extends L2CharacterAI implements Runnable {
	...
	private boolean autoAttackCondition(L2Character target) {
		...
		/*
		 * if ((target != null) && target.isInvul()) { // However EffectInvincible requires to check GMs specially if (target instanceof L2PcInstance && ((L2PcInstance) target).isGM()) return false; if (target instanceof L2Summon && ((L2Summon) target).getOwner().isGM()) return false; }
		 */// rocknow-Fix-Invul
	...
	private final void factionNotifyAndSupport() {
		 ...
		 */// rocknow-Fix-Invul // speeding it up for siege guards
