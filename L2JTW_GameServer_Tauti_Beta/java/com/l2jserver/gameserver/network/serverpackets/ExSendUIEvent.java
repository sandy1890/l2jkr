...
public class ExSendUIEvent extends L2GameServerPacket
{
	...
	protected void writeImpl()
	{
		...
		writeD(0x00); // unknown
		/*
		 * l2jtw start writeS(_isIncrease ? "1" : "0"); // "0": count negative, "1": count positive writeS(String.valueOf(_startTime / 60)); // timer starting minute(s) writeS(String.valueOf(_startTime % 60)); // timer starting second(s) writeS(_text); // text above timer
		 * writeS(String.valueOf(_endTime / 60)); // timer length minute(s) (timer will disappear 10 seconds before it ends) writeS(String.valueOf(_endTime % 60)); // timer length second(s) (timer will disappear 10 seconds before it ends)
		 */
