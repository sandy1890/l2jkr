...
public final class RequestJoinParty extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		if ((target.getClient() == null) || target.getClient().isDetached()) {
			/*
			 * Move To MessageTable For L2JTW requestor.sendMessage("Player is in offline mode.");
			 */
		...
		if (target.isInJail() || requestor.isInJail()) {
			/*
			 * Move To MessageTable For L2JTW requestor.sendMessage("You cannot invite a player while is in Jail.");
			 */
		...
		if (!requestor.isInParty()) {
			...
			if (requestor.getParty().isInDimensionalRift()) {
				/*
				 * Move To MessageTable For L2JTW requestor.sendMessage("You cannot invite a player when you are in the Dimensional Rift.");
				 */
	...
	private void addTargetToParty(L2PcInstance target, L2PcInstance requestor) {
		...
		if (party.getMemberCount() >= 7) // rocknow-God
		{
