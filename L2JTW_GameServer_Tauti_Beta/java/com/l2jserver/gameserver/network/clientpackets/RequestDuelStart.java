	...
	protected void runImpl() {
		...
		if (_partyDuel == 1) {
			...
			if (!activeChar.isInParty() || !(activeChar.isInParty() && activeChar.getParty().isLeader(activeChar))) {
				/*
				 * Move To MessageTable For L2JTW activeChar.sendMessage("You have to be the leader of a party in order to request a party duel.");
				 */
			...
			// Target may not be of the same party
				...
				/*
				 * Move To MessageTable For L2JTW activeChar.sendMessage("This player is a member of your own party.");
				 */
			...
			for (L2PcInstance temp : activeChar.getParty().getMembers()) {
				if (!temp.canDuel()) {
					/*
					 * Move To MessageTable For L2JTW activeChar.sendMessage("Not all the members of your party are ready for a duel.");
					 */
