...
public final class TradeRequest extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		if (!player.getAccessLevel().allowTransaction()) {
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("Transactions are disabled for your current Access Level.");
			 */
		...
		if (partner.isInOlympiadMode() || player.isInOlympiadMode()) {
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("A user currently participating in the Olympiad cannot accept or request a trade.");
			 */
		...
		if (!Config.ALT_GAME_KARMA_PLAYER_CAN_TRADE && (player.getKarma() > 0)) {
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("You cannot trade while you are in a chaotic state.");
			 */
		...
		if (!Config.ALT_GAME_KARMA_PLAYER_CAN_TRADE && (partner.getKarma() > 0)) {
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("You cannot request a trade while your target is in a chaotic state.");
			 */
		...
		if (Config.JAIL_DISABLE_TRANSACTION && (player.isInJail() || partner.isInJail())) {
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("You cannot trade while you are in in Jail.");
			 */
		...
		if (partner.getTradeRefusal()) {
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("That person is in trade refusal mode.");
			 */
