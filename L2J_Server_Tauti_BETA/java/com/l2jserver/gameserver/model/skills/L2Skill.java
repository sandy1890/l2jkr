...
public abstract class L2Skill implements IChanceSkillTrigger {
	...
	private/* final */String _name; // By ShanSoft
	...
	private final int[] _negateCasterId; // cancels the effect of skill ID on caster Add by pmq Quest 144
	...
	public final double getPower(L2Character activeChar, L2Character target, boolean isPvP, boolean isPvE) {
		...
		switch (_skillType) {
			case DEATHLINK: {
				...
				/*
				 * DrHouse: Rolling back to old formula (look below) for DEATHLINK due to this one based on logarithm is not accurate enough. Commented here because probably is a matter of just adjusting a constant if(activeChar.getCurrentHp() / activeChar.getMaxHp() > 0.005) return
				 * _power*(-0.45*Math.log(activeChar.getCurrentHp()/activeChar.getMaxHp())+1.); else return _power*(-0.45*Math.log(0.005)+1.);
				 */
			}
			case FATAL: {
	...
	public final L2Object[] getTargetList(L2Character activeChar, boolean onlyFirst, L2Character target) {
		...
		/*
		 * Move To MessageTable For L2JTW activeChar.sendMessage("Target type of skill is not currently handled");
		 */
	 ...
	 * and usual skill PvP check. If TvT event is active - performing additional checks. Caution: distance is not checked.
	 */
	public static final boolean checkForAreaOffensiveSkills(L2Character caster, L2Character target, L2Skill skill, boolean sourceInArena) {
		...
		if (player != null) {
			if (targetPlayer != null) {
				...
				if (skill.isOffensive() && (player.getSiegeState() > 0) && player.isInsideZone(L2Character.ZONE_SIEGE) && (player.getSiegeState() == targetPlayer.getSiegeState()) && (player.getSiegeSide() == targetPlayer.getSiegeSide())) {
					return false;
				}
