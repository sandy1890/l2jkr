...
public class SevenSignsFestival implements SpawnListener {
	...
	public static final String getFestivalName(int festivalID) {
		...
		switch (festivalID) {
			case FESTIVAL_LEVEL_MAX_31:
				/*
				 * Move To MessageTable For L2JTW festivalName = "Level 31 or lower";
				 */
				festivalName = MessageTable.Messages[25].getMessage();
			break;
			case FESTIVAL_LEVEL_MAX_42:
				/*
				 * Move To MessageTable For L2JTW festivalName = "Level 42 or lower";
				 */
				festivalName = MessageTable.Messages[26].getMessage();
			break;
			case FESTIVAL_LEVEL_MAX_53:
				/*
				 * Move To MessageTable For L2JTW festivalName = "Level 53 or lower";
				 */
				festivalName = MessageTable.Messages[27].getMessage();
			break;
			case FESTIVAL_LEVEL_MAX_64:
				/*
				 * Move To MessageTable For L2JTW festivalName = "Level 64 or lower";
				 */
				festivalName = MessageTable.Messages[28].getMessage();
			break;
			default:
				/*
				 * Move To MessageTable For L2JTW festivalName = "No Level Limit";
				 */
				festivalName = MessageTable.Messages[29].getMessage();
			break;
		}
	...
	public final String getTimeToNextFestivalStr() {
		if (SevenSigns.getInstance().isSealValidationPeriod()) {
			/*
			 * Move To MessageTable For L2JTW return "<font color=\"FF0000\">This is the Seal Validation period. Festivals will resume next week.</font>";
			 */
			return "<font color=\"FF0000\">" + MessageTable.Messages[30].getMessage() + "</font>";
		}
		
		/*
		 * Move To MessageTable For L2JTW return "<font color=\"FF0000\">The next festival will begin in " + getMinsToNextFestival() + " minute(s).</font>";
		 */
		return "<font color=\"FF0000\">" + MessageTable.Messages[31].getExtra(1) + getMinsToNextFestival() + MessageTable.Messages[31].getExtra(2) + "</font>";
	}
	...
	public void setParticipants(int oracle, int festivalId, L2Party festivalParty) {
		...
		if (festivalParty != null) {
			...
			if (Config.DEBUG) {
				_log.info("SevenSignsFestival: " + festivalParty.getMembers().size() + " have signed up to the " + SevenSigns.getCabalShortName(oracle) + " " + getFestivalName(festivalId) + " festival.");
			}
		}
		...
		public synchronized void run() {
			try {
				...
				if (Config.DEBUG) {
					...
					/*
					 * Move To MessageTable For L2JTW sendMessageToAll("Festival Guide", NpcStringId.THE_MAIN_EVENT_WILL_START_IN_2_MINUTES_PLEASE_REGISTER_NOW);
					 */
					sendMessageToAll(MessageTable.Messages[32].getExtra(1), NpcStringId.THE_MAIN_EVENT_WILL_START_IN_2_MINUTES_PLEASE_REGISTER_NOW);
				}
				...
				/*
				 * Move To MessageTable For L2JTW sendMessageToAll("Festival Guide", NpcStringId.THE_MAIN_EVENT_IS_NOW_STARTING);
				 */
				sendMessageToAll(MessageTable.Messages[33].getExtra(1), NpcStringId.THE_MAIN_EVENT_IS_NOW_STARTING);
				...
				for (L2DarknessFestival festivalInst : _festivalInstances.values()) {
					...
					if (end == 2) {
						...
						/*
						 * Move To MessageTable For L2JTW festivalInst.sendMessageToParticipants("The Festival of Darkness will end in " + end + " minute(s).");
						 */
						festivalInst.sendMessageToParticipants(end + MessageTable.Messages[35].getExtra(1));
					}
				}
				...
				for (L2DarknessFestival festivalInst : _festivalInstances.values()) {
					...
					/*
					 * Move To MessageTable For L2JTW festivalInst.sendMessageToParticipants("The chests have spawned! Be quick, the festival will end soon."); //FIXME What is the correct npcString?
					 */
					festivalInst.sendMessageToParticipants(MessageTable.Messages[36].getMessage());
				}
				...
				/*
				 * Move To MessageTable For L2JTW sendMessageToAll("Festival Witch", NpcStringId.THAT_WILL_DO_ILL_MOVE_YOU_TO_THE_OUTSIDE_SOON);
				 */
				sendMessageToAll(MessageTable.Messages[37].getExtra(1), NpcStringId.THAT_WILL_DO_ILL_MOVE_YOU_TO_THE_OUTSIDE_SOON);
	...
	private class L2DarknessFestival {
		...
		public void sendMessageToParticipants(NpcStringId npcStringId) {
			if ((_participants != null) && !_participants.isEmpty()) {
				/*
				 * Move To MessageTable For L2JTW _witchInst.broadcastPacket(new CreatureSay(_witchInst.getObjectId(), Say2.ALL, "Festival Witch", npcStringId));
				 */
				_witchInst.broadcastPacket(new CreatureSay(_witchInst.getObjectId(), Say2.ALL, MessageTable.Messages[39].getMessage(), npcStringId));
			}
		}
		...
		public void sendMessageToParticipants(String npcString) {
			if ((_participants != null) && !_participants.isEmpty()) {
				/*
				 * Move To MessageTable For L2JTW _witchInst.broadcastPacket(new CreatureSay(_witchInst.getObjectId(), Say2.ALL, "Festival Witch", npcString));
				 */
				_witchInst.broadcastPacket(new CreatureSay(_witchInst.getObjectId(), Say2.ALL, MessageTable.Messages[39].getMessage(), npcString));
			}
		}
			...
			if ((_participants != null) && !_participants.isEmpty()) {
				for (int participantObjId : _participants) {
					try {
						...
						/*
						 * Move To MessageTable For L2JTW participant.sendMessage("The festival has ended. Your party leader must now register your score before the next festival takes place.");
						 */
						participant.sendMessage(40);
					} catch (NullPointerException e) {
					}
				}
		...
		public void relocatePlayer(L2PcInstance participant, boolean isRemoving) {
			try {
				...
				/*
				 * Move To MessageTable For L2JTW participant.sendMessage("You have been removed from the festival arena.");
				 */
				participant.sendMessage(41);
			} catch (Exception e) {
