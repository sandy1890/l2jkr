...
public class Castle {
	...
	public void setTaxPercent(L2PcInstance activeChar, int taxPercent) {
		...
		if ((taxPercent < 0) || (taxPercent > maxTax)) {
			/*
			 * Move To MessageTable For L2JTW activeChar.sendMessage("Tax value must be between 0 and " + maxTax + ".");
			 */
			activeChar.sendMessage(MessageTable.Messages[420].getExtra(1) + maxTax + MessageTable.Messages[420].getExtra(2));
			return;
		}
		
		setTaxPercent(taxPercent);
		/*
		 * Move To MessageTable For L2JTW activeChar.sendMessage(getName() + " castle tax changed to " + taxPercent + "%.");
		 */
		activeChar.sendMessage(getName() + MessageTable.Messages[421].getExtra(1) + taxPercent + MessageTable.Messages[421].getExtra(2));
	}
	...
	private void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				// Add By Tiger 2010/01/13 Castle Name Multi language
				switch (getCastleId()) {
					case 1:
						_cname = MessageTable.Messages[422].getMessage();
					break;
					case 2:
						_cname = MessageTable.Messages[423].getMessage();
					break;
					case 3:
						_cname = MessageTable.Messages[424].getMessage();
					break;
					case 4:
						_cname = MessageTable.Messages[425].getMessage();
					break;
					case 5:
						_cname = MessageTable.Messages[426].getMessage();
					break;
					case 6:
						_cname = MessageTable.Messages[427].getMessage();
					break;
					case 7:
						_cname = MessageTable.Messages[428].getMessage();
					break;
					case 8:
						_cname = MessageTable.Messages[429].getMessage();
					break;
					case 9:
						_cname = MessageTable.Messages[430].getMessage();
					break;
					default:
						_cname = MessageTable.Messages[431].getMessage();
				}
	...
	private void loadDoor() {
		...
		if (Config.DEBUG) {
			_log.info(this + "성 " + _doors.size() + "개 문이 로드되었습니다.");
		}
	}
