...
public class Siege implements Siegable {
		...
		public void run() {
			...
			try {
				...
				if (timeRemaining > 86400000) {
					...
					sm.addString(getCastle().getCName()); // Update by pmq
	...
	public void registerDefender(L2PcInstance player, boolean force) {
		if (getCastle().getOwnerId() <= 0) {
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("You cannot register as a defender because " + getCastle().getName() + " is owned by NPC.");
			 */
			player.sendMessage(MessageTable.Messages[454].getExtra(1) + getCastle().getName() + MessageTable.Messages[454].getExtra(2));
