...
public class FortSiege implements Siegable {
	...
	public boolean checkIfCanRegister(L2PcInstance player) {
		...
		if ((player.getClan() == null) || (player.getClan().getLevel() < FortSiegeManager.getInstance().getSiegeClanMinLevel())) {
			b = false;
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("Only clans with Level " + FortSiegeManager.getInstance().getSiegeClanMinLevel() + " and higher may register for a fortress siege.");
			 */
			player.sendMessage(MessageTable.Messages[441].getExtra(1) + FortSiegeManager.getInstance().getSiegeClanMinLevel() + MessageTable.Messages[441].getExtra(2));
			...
			b = false;
			/*
			 * Move To MessageTable For L2JTW player.sendMessage("You need 250,000 adena to register"); // replace me with html
			 */
			player.sendMessage(442); // replace me with html
		} else {
