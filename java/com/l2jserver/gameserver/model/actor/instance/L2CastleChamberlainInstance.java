		...
		if (condition <= COND_ALL_FALSE) {
			...
			if (actualCommand.equalsIgnoreCase("banish_foreigner")) {
				...
				if (player.isClanLeader()) {
					...
					html.replace("%castlename%", getCastle().getCName()); // update by pmq
					
					int currentPeriod = SevenSigns.getInstance().getCurrentPeriod();
					switch (currentPeriod) {
						case SevenSigns.PERIOD_COMP_RECRUITING:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_event%", "Quest Event Initialization");
							 */
							html.replace("%ss_event%", MessageTable.Messages[543].getMessage());
						break;
						case SevenSigns.PERIOD_COMPETITION:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_event%", "Competition (Quest Event)");
							 */
							html.replace("%ss_event%", MessageTable.Messages[544].getMessage());
						break;
						case SevenSigns.PERIOD_COMP_RESULTS:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_event%", "Quest Event Results");
							 */
							html.replace("%ss_event%", MessageTable.Messages[545].getMessage());
						break;
						case SevenSigns.PERIOD_SEAL_VALIDATION:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_event%", "Seal Validation");
							 */
							html.replace("%ss_event%", MessageTable.Messages[546].getMessage());
						break;
					}
					...
					switch (sealOwner1) {
						case SevenSigns.CABAL_NULL:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_avarice%", "Not in Possession");
							 */
							html.replace("%ss_avarice%", MessageTable.Messages[547].getMessage());
						break;
						case SevenSigns.CABAL_DAWN:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_avarice%", "Lords of Dawn");
							 */
							html.replace("%ss_avarice%", MessageTable.Messages[548].getMessage());
						break;
						case SevenSigns.CABAL_DUSK:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_avarice%", "Revolutionaries of Dusk");
							 */
							html.replace("%ss_avarice%", MessageTable.Messages[549].getMessage());
						break;
					}
					...
					switch (sealOwner2) {
						case SevenSigns.CABAL_NULL:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_gnosis%", "Not in Possession");
							 */
							html.replace("%ss_gnosis%", MessageTable.Messages[547].getMessage());
						break;
						case SevenSigns.CABAL_DAWN:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_gnosis%", "Lords of Dawn");
							 */
							html.replace("%ss_gnosis%", MessageTable.Messages[548].getMessage());
						break;
						case SevenSigns.CABAL_DUSK:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_gnosis%", "Revolutionaries of Dusk");
							 */
							html.replace("%ss_gnosis%", MessageTable.Messages[549].getMessage());
						break;
					}
					...
					switch (sealOwner3) {
						case SevenSigns.CABAL_NULL:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_strife%", "Not in Possession");
							 */
							html.replace("%ss_strife%", MessageTable.Messages[547].getMessage());
						break;
						case SevenSigns.CABAL_DAWN:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_strife%", "Lords of Dawn");
							 */
							html.replace("%ss_strife%", MessageTable.Messages[548].getMessage());
						break;
						case SevenSigns.CABAL_DUSK:
							/*
							 * Move To MessageTable For L2JTW html.replace("%ss_strife%", "Revolutionaries of Dusk");
							 */
							html.replace("%ss_strife%", MessageTable.Messages[549].getMessage());
						break;
					}
					player.sendPacket(html);
				} else {
			...
			// control
			{
				if ((player.getClanPrivileges() & L2Clan.CP_CS_TAXES) == L2Clan.CP_CS_TAXES) {
					...
					/*
					 * Move To MessageTable For L2JTW final String msg = StringUtil.concat("<html><body>", getName(), ":<br>" + "Current tax rate: ", String.valueOf(getCastle().getTaxPercent()), "%<br>" + "<table>" + "<tr>" + "<td>Change tax rate to:</td>" + "<td><edit var=\"value\" width=40><br>" +
					 * "<button value=\"Adjust\" action=\"bypass -h npc_%objectId%_tax_set $value\" width=80 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>" + "</tr>" + "</table>" + "</center>" + "</body></html>");
					 */
					final String msg = StringUtil.concat("<html><body>", getName(), ":<br>" + MessageTable.Messages[550].getMessage(), String.valueOf(getCastle().getTaxPercent()), "%<br>" + "<table>" + "<tr>" + "<td>" + MessageTable.Messages[551].getMessage() + "</td>" + "<td><edit var=\"value\" width=40><br>" + "<button value=\"" + MessageTable.Messages[552].getMessage() + "\" action=\"bypass -h npc_%objectId%_tax_set $value\" width=80 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>" + "</tr>" + "</table>" + "</center>" + "</body></html>");
					...
					if (val.equalsIgnoreCase("recovery")) {
						if (st.countTokens() >= 1) {
							if (getCastle().getOwnerId() == 0) {
								/*
								 * Move To MessageTable For L2JTW player.sendMessage("This castle have no owner, you cannot change configuration");
								 */
								player.sendMessage(553);
								return;
							}
							val = st.nextToken();
							if (val.equalsIgnoreCase("hp_cancel")) {
								...
								/*
								 * Move To MessageTable For L2JTW html.replace("%name%", "Fireplace (HP Recovery Device)");
								 */
								html.replace("%name%", MessageTable.Messages[556].getMessage());
								...
								/*
								 * Move To MessageTable For L2JTW html.replace("%cost%", String.valueOf(cost) + "</font>Adena /" + String.valueOf(Config.CS_HPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day</font>)"); html.replace("%use%",
								 * "Provides additional HP recovery for clan members in the castle.<font color=\"00FFFF\">" + String.valueOf(percent) + "%</font>");
								 */
								html.replace("%cost%", String.valueOf(cost) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_HPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + "</font>)");
								html.replace("%use%", MessageTable.Messages[557].getMessage() + "<font color=\"00FFFF\">" + String.valueOf(percent) + "%</font>");
								...
								html.setFile(player.getHtmlPrefix(), "data/html/chamberlain/functions-apply.htm");
								/*
								 * Move To MessageTable For L2JTW html.replace("%name%", "Carpet (MP Recovery)");
								 */
								html.replace("%name%", MessageTable.Messages[558].getMessage());
								int percent = Integer.parseInt(val);
								int cost;
								switch (percent) {
									case 5:
								...
								/*
								 * Move To MessageTable For L2JTW html.replace("%cost%", String.valueOf(cost) + "</font>Adena /" + String.valueOf(Config.CS_MPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day</font>)"); html.replace("%use%",
								 * "Provides additional MP recovery for clan members in the castle.<font color=\"00FFFF\">" + String.valueOf(percent) + "%</font>");
								 */
								html.replace("%cost%", String.valueOf(cost) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_MPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + "</font>)");
								html.replace("%use%", MessageTable.Messages[559].getMessage() + "<font color=\"00FFFF\">" + String.valueOf(percent) + "%</font>");
								...
								html.setFile(player.getHtmlPrefix(), "data/html/chamberlain/functions-apply.htm");
								/*
								 * Move To MessageTable For L2JTW html.replace("%name%", "Chandelier (EXP Recovery Device)");
								 */
								html.replace("%name%", MessageTable.Messages[560].getMessage());
								int percent = Integer.parseInt(val);
								int cost;
								switch (percent) {
									case 15:
								...
								/*
								 * Move To MessageTable For L2JTW html.replace("%cost%", String.valueOf(cost) + "</font>Adena /" + String.valueOf(Config.CS_EXPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day</font>)"); html.replace("%use%",
								 * "Restores the Exp of any clan member who is resurrected in the castle.<font color=\"00FFFF\">" + String.valueOf(percent) + "%</font>");
								 */
								html.replace("%cost%", String.valueOf(cost) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_EXPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + "</font>)");
								html.replace("%use%", MessageTable.Messages[561].getMessage() + "<font color=\"00FFFF\">" + String.valueOf(percent) + "%</font>");
						...
						if (getCastle().getFunction(Castle.FUNC_RESTORE_HP) != null) {
							/*
							 * Move To MessageTable For L2JTW html.replace("%hp_recovery%", String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_HP).getLvl()) + "%</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_HP).getLease()) + "</font>Adena /" +
							 * String.valueOf(Config.CS_HPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day)"); html.replace("%hp_period%", "Withdraw the fee for the next time at " + format.format(getCastle().getFunction(Castle.FUNC_RESTORE_HP).getEndTime())); html.replace("%change_hp%",
							 * "[<a action=\"bypass -h npc_%objectId%_manage recovery hp_cancel\">Deactivate</a>]" + hp);
							 */
							html.replace("%hp_recovery%", String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_HP).getLvl()) + "%</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_HP).getLease()) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_HPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + ")");
							html.replace("%hp_period%", MessageTable.Messages[562].getMessage() + format.format(getCastle().getFunction(Castle.FUNC_RESTORE_HP).getEndTime()));
							html.replace("%change_hp%", "[<a action=\"bypass -h npc_%objectId%_manage recovery hp_cancel\">" + MessageTable.Messages[563].getMessage() + "</a>]" + hp);
						} else {
							/*
							 * Move To MessageTable For L2JTW html.replace("%hp_recovery%", "none"); html.replace("%hp_period%", "none");
							 */
							html.replace("%hp_recovery%", MessageTable.Messages[564].getMessage());
							html.replace("%hp_period%", MessageTable.Messages[564].getMessage());
							html.replace("%change_hp%", hp);
						}
						if (getCastle().getFunction(Castle.FUNC_RESTORE_EXP) != null) {
							/*
							 * Move To MessageTable For L2JTW html.replace("%exp_recovery%", String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_EXP).getLvl()) + "%</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_EXP).getLease()) +
							 * "</font>Adena /" + String.valueOf(Config.CS_EXPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day)"); html.replace("%exp_period%", "Withdraw the fee for the next time at " + format.format(getCastle().getFunction(Castle.FUNC_RESTORE_EXP).getEndTime()));
							 * html.replace("%change_exp%", "[<a action=\"bypass -h npc_%objectId%_manage recovery exp_cancel\">Deactivate</a>]" + exp);
							 */
							html.replace("%exp_recovery%", String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_EXP).getLvl()) + "%</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_EXP).getLease()) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_EXPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + ")");
							html.replace("%exp_period%", MessageTable.Messages[562].getMessage() + format.format(getCastle().getFunction(Castle.FUNC_RESTORE_EXP).getEndTime()));
							html.replace("%change_exp%", "[<a action=\"bypass -h npc_%objectId%_manage recovery exp_cancel\">" + MessageTable.Messages[563].getMessage() + "</a>]" + exp);
						} else {
							/*
							 * Move To MessageTable For L2JTW html.replace("%exp_recovery%", "none"); html.replace("%exp_period%", "none");
							 */
							html.replace("%exp_recovery%", MessageTable.Messages[564].getMessage());
							html.replace("%exp_period%", MessageTable.Messages[564].getMessage());
							html.replace("%change_exp%", exp);
						}
						if (getCastle().getFunction(Castle.FUNC_RESTORE_MP) != null) {
							/*
							 * Move To MessageTable For L2JTW html.replace("%mp_recovery%", String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_MP).getLvl()) + "%</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_MP).getLease()) + "</font>Adena /" +
							 * String.valueOf(Config.CS_MPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day)"); html.replace("%mp_period%", "Withdraw the fee for the next time at " + format.format(getCastle().getFunction(Castle.FUNC_RESTORE_MP).getEndTime())); html.replace("%change_mp%",
							 * "[<a action=\"bypass -h npc_%objectId%_manage recovery mp_cancel\">Deactivate</a>]" + mp);
							 */
							html.replace("%mp_recovery%", String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_MP).getLvl()) + "%</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_RESTORE_MP).getLease()) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_MPREG_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + ")");
							html.replace("%mp_period%", MessageTable.Messages[562].getMessage() + format.format(getCastle().getFunction(Castle.FUNC_RESTORE_MP).getEndTime()));
							html.replace("%change_mp%", "[<a action=\"bypass -h npc_%objectId%_manage recovery mp_cancel\">" + MessageTable.Messages[563].getMessage() + "</a>]" + mp);
						} else {
							/*
							 * Move To MessageTable For L2JTW html.replace("%mp_recovery%", "none"); html.replace("%mp_period%", "none");
							 */
							html.replace("%mp_recovery%", MessageTable.Messages[564].getMessage());
							html.replace("%mp_period%", MessageTable.Messages[564].getMessage());
						...
						if (st.countTokens() >= 1) {
							...
							if (val.equalsIgnoreCase("tele_cancel")) {
								...
								html.setFile(player.getHtmlPrefix(), "data/html/chamberlain/functions-apply.htm");
								/*
								 * Move To MessageTable For L2JTW html.replace("%name%", "Insignia (Supplementary Magic)");
								 */
								html.replace("%name%", MessageTable.Messages[565].getMessage());
								...
								/*
								 * Move To MessageTable For L2JTW html.replace("%cost%", String.valueOf(cost) + "</font>Adena /" + String.valueOf(Config.CS_SUPPORT_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day</font>)"); html.replace("%use%", "Enables the use of supplementary magic.");
								 */
								html.replace("%cost%", String.valueOf(cost) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_SUPPORT_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + "</font>)");
								html.replace("%use%", MessageTable.Messages[566].getMessage());
								...
								html.setFile(player.getHtmlPrefix(), "data/html/chamberlain/functions-apply.htm");
								/*
								 * Move To MessageTable For L2JTW html.replace("%name%", "Mirror (Teleportation Device)");
								 */
								html.replace("%name%", MessageTable.Messages[567].getMessage());
								int stage = Integer.parseInt(val);
								int cost;
								switch (stage) {
									case 1:
								...
								/*
								 * Move To MessageTable For L2JTW html.replace("%cost%", String.valueOf(cost) + "</font>Adena /" + String.valueOf(Config.CS_TELE_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day</font>)"); html.replace("%use%",
								 * "Teleports clan members in a castle to the target <font color=\"00FFFF\">Stage " + String.valueOf(stage) + "</font> staging area"); html.replace("%apply%", "other tele " + String.valueOf(stage));
								 */
								html.replace("%cost%", String.valueOf(cost) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_TELE_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + "</font>)");
								html.replace("%use%", MessageTable.Messages[568].getMessage() + "<font color=\"00FFFF\">" + MessageTable.Messages[569].getMessage() + String.valueOf(stage) + "</font>" + MessageTable.Messages[570].getMessage());
								...
								if (st.countTokens() >= 1) {
									...
									if (getCastle().getFunction(Castle.FUNC_TELEPORT) != null) {
										if (getCastle().getFunction(Castle.FUNC_TELEPORT).getLvl() == Integer.parseInt(val)) {
											html.setFile(player.getHtmlPrefix(), "data/html/chamberlain/functions-used.htm");
											/*
											 * Move To MessageTable For L2JTW html.replace("%val%", "Stage " + String.valueOf(val));
											 */
											html.replace("%val%", MessageTable.Messages[569].getMessage() + String.valueOf(val));
											sendHtmlMessage(player, html);
											return;
										}
									}
						...
						/*
						 * Move To MessageTable For L2JTW String tele = "[<a action=\"bypass -h npc_%objectId%_manage other edit_tele 1\">Level 1</a>][<a action=\"bypass -h npc_%objectId%_manage other edit_tele 2\">Level 2</a>]"; String support =
						 ...
						 * ;
						 */
						String tele = "[<a action=\"bypass -h npc_%objectId%_manage other edit_tele 1\">" + MessageTable.Messages[571].getMessage() + "1</a>][<a action=\"bypass -h npc_%objectId%_manage other edit_tele 2\">" + MessageTable.Messages[571].getMessage() + "2</a>]";
						String support = "[<a action=\"bypass -h npc_%objectId%_manage other edit_support 1\">" + MessageTable.Messages[571].getMessage() + "1</a>][<a action=\"bypass -h npc_%objectId%_manage other edit_support 2\">" + MessageTable.Messages[571].getMessage() + "2</a>][<a action=\"bypass -h npc_%objectId%_manage other edit_support 3\">" + MessageTable.Messages[571].getMessage() + "3</a>][<a action=\"bypass -h npc_%objectId%_manage other edit_support 4\">" + MessageTable.Messages[571].getMessage() + "4</a>]";
						if (getCastle().getFunction(Castle.FUNC_TELEPORT) != null) {
							/*
							 * Move To MessageTable For L2JTW html.replace("%tele%", "Stage " + String.valueOf(getCastle().getFunction(Castle.FUNC_TELEPORT).getLvl()) + "</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_TELEPORT).getLease()) + "</font>Adena /" +
							 * String.valueOf(Config.CS_TELE_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day)"); html.replace("%tele_period%", "Withdraw the fee for the next time at " + format.format(getCastle().getFunction(Castle.FUNC_TELEPORT).getEndTime())); html.replace("%change_tele%",
							 * "[<a action=\"bypass -h npc_%objectId%_manage other tele_cancel\">Deactivate</a>]" + tele);
							 */
							html.replace("%tele%", MessageTable.Messages[569].getMessage() + String.valueOf(getCastle().getFunction(Castle.FUNC_TELEPORT).getLvl()) + "</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_TELEPORT).getLease()) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_TELE_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + ")");
							html.replace("%tele_period%", MessageTable.Messages[562].getMessage() + format.format(getCastle().getFunction(Castle.FUNC_TELEPORT).getEndTime()));
							html.replace("%change_tele%", "[<a action=\"bypass -h npc_%objectId%_manage other tele_cancel\">" + MessageTable.Messages[563].getMessage() + "</a>]" + tele);
						} else {
							/*
							 * Move To MessageTable For L2JTW html.replace("%tele%", "none"); html.replace("%tele_period%", "none");
							 */
							html.replace("%tele%", MessageTable.Messages[564].getMessage());
							html.replace("%tele_period%", MessageTable.Messages[564].getMessage());
							html.replace("%change_tele%", tele);
						}
						if (getCastle().getFunction(Castle.FUNC_SUPPORT) != null) {
							/*
							 * Move To MessageTable For L2JTW html.replace("%support%", "Stage " + String.valueOf(getCastle().getFunction(Castle.FUNC_SUPPORT).getLvl()) + "</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_SUPPORT).getLease()) + "</font>Adena /" +
							 * String.valueOf(Config.CS_SUPPORT_FEE_RATIO / 1000 / 60 / 60 / 24) + " Day)"); html.replace("%support_period%", "Withdraw the fee for the next time at " + format.format(getCastle().getFunction(Castle.FUNC_SUPPORT).getEndTime())); html.replace("%change_support%",
							 * "[<a action=\"bypass -h npc_%objectId%_manage other support_cancel\">Deactivate</a>]" + support);
							 */
							html.replace("%support%", MessageTable.Messages[569].getMessage() + String.valueOf(getCastle().getFunction(Castle.FUNC_SUPPORT).getLvl()) + "</font> (<font color=\"FFAABB\">" + String.valueOf(getCastle().getFunction(Castle.FUNC_SUPPORT).getLease()) + "</font>" + MessageTable.Messages[554].getMessage() + " /" + String.valueOf(Config.CS_SUPPORT_FEE_RATIO / 1000 / 60 / 60 / 24) + MessageTable.Messages[555].getMessage() + ")");
							html.replace("%support_period%", MessageTable.Messages[562].getMessage() + format.format(getCastle().getFunction(Castle.FUNC_SUPPORT).getEndTime()));
							html.replace("%change_support%", "[<a action=\"bypass -h npc_%objectId%_manage other support_cancel\">" + MessageTable.Messages[563].getMessage() + "</a>]" + support);
						} else {
							/*
							 * Move To MessageTable For L2JTW html.replace("%support%", "none"); html.replace("%support_period%", "none");
							 */
							html.replace("%support%", MessageTable.Messages[564].getMessage());
							html.replace("%support_period%", MessageTable.Messages[564].getMessage());
				...
				try {
					...
					try {
						...
						/*
						 * Move To MessageTable For L2JTW player.sendMessage("Invalid skill level, contact your admin!");
						 */
						player.sendMessage(572);
					}
				} catch (Exception e) {
					/*
					 * Move To MessageTable For L2JTW player.sendMessage("Invalid skill level, contact your admin!");
			...
			} else if (actualCommand.equalsIgnoreCase("siege_time_set")) // set preDay
			{
				...
				if (player.isClanLeader()) {
					if (player.getInventory().getItemByItemId(6841) == null) {
						...
						html.replace("%FeudName%", String.valueOf(getCastle().getCName())); // Update by pmq
					} else {
