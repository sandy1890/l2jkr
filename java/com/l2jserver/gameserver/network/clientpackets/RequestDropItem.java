...
public final class RequestDropItem extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		if ((item == null) || (_count == 0) || !activeChar.validateItemManipulation(_objectId, "drop") || (!Config.ALLOW_DISCARDITEM && !activeChar.isGM()) || (!item.isDropable() && !(activeChar.isGM() && Config.GM_TRADE_RESTRICTED_ITEMS)) || ((item.getItemType() == L2EtcItemType.PET_COLLAR) && activeChar.havePetInvItems()) || activeChar.isInsideZone(L2Character.ZONE_NOITEMDROP))
		{
		...
		if (Config.JAIL_DISABLE_TRANSACTION && activeChar.isInJail()) {
			/*
			 * Move To MessageTable For L2JTW activeChar.sendMessage("You cannot drop items in Jail.");
			 */
		...
		if (!activeChar.getAccessLevel().allowTransaction()) {
			/*
			 * Move To MessageTable For L2JTW activeChar.sendMessage("Transactions are disabled for your Access Level.");
			 */
		...
		if (!activeChar.getInventory().canManipulateWithItemId(item.getItemId())) {
			/*
			 * Move To MessageTable For L2JTW activeChar.sendMessage("You cannot use this item.");
			 */
