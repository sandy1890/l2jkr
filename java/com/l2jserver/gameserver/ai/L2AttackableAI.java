...
public class L2AttackableAI extends L2CharacterAI implements Runnable {
	 ...
	 * <BR>
	 * <li>The target isn't a Folk, a Door or another L2Npc</li> <li>The target isn't dead, isn't invulnerable, isn't in silent moving mode AND too far (>100)</li> <li>The target is in the actor Aggro range and is at the same height</li> <li>The L2PcInstance target has karma (=PK)</li><BR>
	 * <BR>
	 * <B><U> Actor is a L2MonsterInstance</U> :</B><BR>
	 * <BR>
	 * <li>The target isn't a Folk, a Door or another L2Npc</li> <li>The target isn't dead, isn't invulnerable, isn't in silent moving mode AND too far (>100)</li> <li>The target is in the actor Aggro range and is at the same height</li> <li>The actor is Aggressive</li><BR>
	 * <BR>
	...
	private boolean autoAttackCondition(L2Character target) {
		...
		/*
		 * if (target.isInvul()) { // However EffectInvincible requires to check GMs specially if (target instanceof L2PcInstance && ((L2PcInstance) target).isGM()) return false; if (target instanceof L2Summon && ((L2Summon) target).getOwner().isGM()) return false; }
		 */// rocknow-Fix-Invul
	...
	private void movementDisable() {
		...
		try {
			...
			if (!_skillrender.getGeneralskills().isEmpty()) {
				...
				/*
				 * for(L2Skill sk:_skillrender.getUniversalSkills()) { if(sk.getMpConsume()>=_actor.getCurrentMp() || _actor.isSkillDisabled(sk.getId()) ||(sk.getCastRange()+ _actor.getTemplate().getCollisionRadius() + getAttackTarget().getTemplate().getCollisionRadius() <= dist2 && !canAura(sk))
				 * ||(sk.isMagic()&&_actor.isMuted()) ||(!sk.isMagic()&&_actor.isPhysicalMuted())) { continue; } if(!GeoData.getInstance().canSeeTarget(_actor,getAttackTarget())) continue; clientStopMoving(null); L2Object target = getAttackTarget(); //_actor.setTarget(_actor); _actor.doCast(sk);
				 * //_actor.setTarget(target); return; }
				 */
			}
			// timepass = timepass + 1;
			if (npc.isMovementDisabled()) {
