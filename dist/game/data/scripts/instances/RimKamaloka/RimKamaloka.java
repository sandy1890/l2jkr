...
public class RimKamaloka extends Quest
{
	...
	class DespawnTask implements Runnable
	{
		...
		public void run()
		{
			if (_world != null
					&& !_world.isFinished
					&& !_world.lastAttack.isEmpty()
					&& !_world.spawnedMobs.isEmpty())
			{
				...
				for (L2MonsterInstance mob : _world.spawnedMobs)
				{
					if (mob == null || mob.isDead() || !mob.isVisible())
						continue;
					if (_world.lastAttack.containsKey(mob.getObjectId())
							&& time - _world.lastAttack.get(mob.getObjectId()) > DESPAWN_DELAY)
					{
						mob.deleteMe();
	...
	public final String onAdvEvent (String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (event.equalsIgnoreCase("Exit"))
		{
			try
			{
				...
				if (world instanceof RimKamaWorld
