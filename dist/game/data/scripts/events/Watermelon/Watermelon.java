...
/**
 * @author JOJO Original: "SquashEvent" Gnacik version 1.0 Update By pmq 04-09-2010
 */
public class Watermelon extends Quest {
	...
	/**
	 * 初章紀念樂器 克魯諾齊塔納 Item ID (4202) * 貳章紀念樂器 克魯諾鈴鼓 Item ID (5133) 參章紀念樂器 克魯諾杖鐘 Item ID (5817) * 肆章紀念樂器 克魯諾達布卡 Item ID (7058) 伍章紀念樂器 克魯諾瑪拉卡斯 Item ID (8350)
	 */
	...
	/**
	 * 未成熟的西瓜 Mob ID (13271) * 優良的西瓜 Mob ID (13273) * 不良的西瓜 Mob ID (13272) 未成熟的香甜西瓜 Mob ID (13275) * 優良的香甜西瓜 Mob ID (13277) * 不良的香甜西瓜 Mob ID (13276) 優良的西瓜王 Mob ID (13274) * 優良的香甜西瓜王 Mob ID (13278)
	 */
	...
	{
		/**
		 * must be sorted by npcId ! npcId, chance, itemId,qty [,itemId,qty...] Young Watermelon 未成熟的西瓜
		 */
		{
			13271,
			100,
		...
		/**
		 * Young Honey Watermelon 未成熟的香甜西瓜
		 */
		{
			13275,
			100,
		...
		/**
		 * Defective Honey Watermelon 不良的香甜西瓜
		 */
		{
			13276,
			50,
		...
		/**
		 * Large Rain Honey Watermelon 優良的香甜西瓜王
		 */
		{
			13278,
			10,
	...
	private static final void dropItem(L2ChronoMonsterInstance mob, L2PcInstance player) {
		...
		for (int[] drop : DROPLIST) {
			/**
			 * npcId = drop[0] chance = drop[1] itemId = drop[2,4,6,8...] itemQty = drop[3,5,7,9...]
			 */
			if (npcId == drop[0]) {
	...
	private void randomSpawn(int king, L2ChronoMonsterInstance mob) {
		final TheInstance self = get(mob);
		if ((++self.nectar > 5) && (self.nectar <= 15) && (Rnd.get(100) < 10)) {
			spawnNext(king, mob);
		} else {
			nectarText(mob);
		}
