...
public class RecipeController {
	...
	public void requestMakeItem(L2PcInstance player, int recipeListId) {
		...
		if (Config.ALT_GAME_CREATION && _activeMakers.containsKey(player.getObjectId())) {
			...
			/*
			 * Move To MessageTable For L2JTW sm.addString("You are busy creating.");
			 */
	...
	private static class RecipeItemMaker implements Runnable {
		...
		public void run() {
			if (!Config.IS_CRAFTING_ENABLED) {
				/*
				 * Move To MessageTable For L2JTW _target.sendMessage("Item creation is currently disabled.");
				 */
			...
			if (Config.ALT_GAME_CREATION && !_activeMakers.containsKey(_player.getObjectId())) {
				if (_target != _player) {
					/*
					 * Move To MessageTable For L2JTW _target.sendMessage("Manufacture aborted"); _player.sendMessage("Manufacture aborted");
					 */
					_target.sendMessage(6);
					_player.sendMessage(6);
				} else {
					/*
					 * Move To MessageTable For L2JTW _player.sendMessage("Item creation aborted");
					 */
		...
		private void grabSomeItems() {
			...
			while ((grabItems > 0) && !_items.isEmpty()) {
				...
				if (_target == _player) {
					/*
					 * Move To MessageTable For L2JTW SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.S1_S2_EQUIPPED); // you equipped ...
					 */
					SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.S2_S1_DISAPPEARED); // you equipped ... //Update by rocknow
					sm.addItemNumber(count);
					sm.addItemName(item.getItemId());
					_player.sendPacket(sm);
				} else {
					/*
					 * Move To MessageTable For L2JTW _target.sendMessage("Manufacturer " + _player.getName() + " used " + count + " " + item.getItemName());
					 */
		...
		private boolean calculateStatUse(boolean isWait, boolean isReduce) {
			...
			for (L2RecipeStatInstance statUse : _recipeList.getStatUse()) {
				...
				if (statUse.getType() == L2RecipeStatInstance.StatType.HP) {
					...
					/*
					 * Move To MessageTable For L2JTW _target.sendMessage("Recipe error!!!, please tell this to your GM.");
					 */
