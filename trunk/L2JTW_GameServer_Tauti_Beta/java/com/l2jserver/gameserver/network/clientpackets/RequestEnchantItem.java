	...
	protected void runImpl()
	{
		...
		synchronized (item)
		{
			...
			if (Rnd.get(100) < chance)
			{
				...
				item.updateDatabase();
				/*
				 * l2jtw start activeChar.sendPacket(new EnchantResult(0, 0, 0));
				 */
				...
				// Add L2JO Enchant4Skill Start
				if ((scroll.getItemId() == 13540) && (item.getEnchantLevel() > 3))
				{
					L2Skill skill1 = SkillTable.getInstance().getInfo(5965, 1);
					if (skill1 != null)
					{
						MagicSkillUse msk = new MagicSkillUse(activeChar, activeChar, 5965, 1, 500, 1500);
						activeChar.broadcastPacket(msk);
					}
				}
				// Add L2JO Enchant4Skill End
			...
			else
			{
				...
				{
					L2Skill skill = SkillTable.getInstance().getInfo(5949, 1);
					if (skill != null)
					{
						MagicSkillUse msk = new MagicSkillUse(activeChar, activeChar, 5949, 1, 500, 1500);
						activeChar.broadcastPacket(msk);
					}
				}
				// Add L2JO Enchant4Skill End
