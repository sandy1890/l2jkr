		...
		if (actualCommand.equalsIgnoreCase("select")) {
			...
			if (val == 2) {
				...
				if (CastleManager.getInstance().getCastleById(castleId).getOwnerId() < 1) {
					/*
					 * Move To MessageTable For L2JTW html.setHtml("<html><body>Contact is currently not possible, "+CastleManager.getInstance().getCastleById(castleId).getName()+" Castle isn't currently owned by clan.</body></html>");
					 */
					html.setHtml("<html><body>" + MessageTable.Messages[653].getMessage() + CastleManager.getInstance().getCastleById(castleId).getName() + MessageTable.Messages[654].getMessage() + "</body></html>");
					player.sendPacket(html);
					return;
				}
			}
