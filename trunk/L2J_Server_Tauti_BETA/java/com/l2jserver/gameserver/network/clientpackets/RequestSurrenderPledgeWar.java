...
public final class RequestSurrenderPledgeWar extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		if (!_clan.isAtWarWith(clan.getClanId())) {
			/*
			 * Move To MessageTable For L2JTW _activeChar.sendMessage("You aren't at war with this clan.");
			 */
		 ...
		 * SystemMessage.getSystemMessage(SystemMessageId.S1_IS_BUSY_TRY_LATER); sm.addString(leader.getName()); player.sendPacket(sm); return; } leader.setTransactionRequester(player); player.setTransactionRequester(leader); leader.sendPacket(new
